...
const objectCounter = (obj) => {
    let counter = obj.reduce(function(acc, current, index){
        return index + 1;
    })
    return counter;
...
const stringReverse = (str) => {
    let reversed = str.split(" ").reduce((acc, current, index) => {
        return `${current} ${acc}`;
    })
    return reversed;
    // write your code here
...
const statistics = (obj) => {
    let output = {};
    obj.reduce((acc, current, index) => {
        (output[current.votes_To] === undefined) ? output[current.votes_To] = 1 : output[current.votes_To] += 1; 
    }, undefined)
    return output;
    // write your code here
}
