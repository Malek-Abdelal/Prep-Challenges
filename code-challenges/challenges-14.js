...
const replaceWords = (str) => {
    let remove =["I", "am", "was"];
    let replace =["We", "are", "were"];
    let arr = str.split(" ");
    for(let ele in remove){
        let toReplace = arr.indexOf(remove[ele]);
        (toReplace !== -1) ? arr.splice(toReplace, 1, replace[ele]) : null;
    }
    return arr.join(" ");
    // write your code here
}
...
const arrToStr = (arr) => {
    let array = [];
    for(let i in arr){
        ((parseInt(i) + 1) % 5 === 0) ? array[i] = `${arr[i]},` :  array[i] = arr[i];
    }
    return array.join(" ");
    // write your code here
}
...
const letterCounter = (str) => {
    ...
    for(let i = 0; i < arr.length; i++){
        (arr[i] === arr[i + 1]) ? x++ : (arr[i] === " ") ? newArr.push(" ") : (newArr.push(`${arr[i]}${x}`), x = 1);
    }
    return newArr.join("");
    // write your code here
}
