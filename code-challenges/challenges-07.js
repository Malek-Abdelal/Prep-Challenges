...
const applicationsStatics = (arr) => {
    ...
    let result = {python_Devs: 0, javaScript_Devs: 0, dotNet_Devs: 0, java_Devs: 0, totalApplicants: 0, rejectedApplicants: 0};

    for (let i in arr) {

        (arr[i].tech === "Python") ? result.python_Devs += 1: (arr[i].tech === "JS") ? result.javaScript_Devs += 1 : (arr[i].tech === ".Net") ? result.dotNet_Devs += 1 :(arr[i].tech === "Java") ? result.java_Devs += 1: null;

        (((arr[i].firstName === ("" || null) && arr[i].lastName === ("" || null))) || arr[i].yearsOfExperience <= 1) ? result.rejectedApplicants += 1 : null;
...
const classesAvg = (data) => {
    ...
    for(let i in data.grades){
        // for(grades[i].classes in data.grades[i] ){
        for (let x in data.grades[i].classes){
            let sum = 0;
            for (let n in data.grades[i].classes[x].classScores){
            sum += data.grades[i].classes[x].classScores[n]; 
            }
